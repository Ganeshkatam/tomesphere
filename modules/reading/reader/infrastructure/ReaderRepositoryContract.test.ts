...
describe('ReaderRepository Contract', () => {
    ...
    it('should save and find a session by id', async () => {
        const session = ReaderSession.start('session-1', 'reader-1', 'book-1', createPos('loc-0', 0));
    ...
    it('should correctly save and restore bookmarks and highlights', async () => {
        ...
        const bookmark = Bookmark.create({ id: 'b1', bookId: 'book-1', readerId: 'reader-1', location: 'loc-5', orderIndex: 0 });
        session.addBookmark(bookmark);

        const highlight = Highlight.create({ id: 'h1', bookId: 'book-1', readerId: 'reader-1', location: 'loc-10', text: 'Important' });
    ...
    it('should find the active session for a reader', async () => {
        const session1 = ReaderSession.start('session-1', 'reader-1', 'book-1', createPos('loc-0', 0));
        session1.complete(createPos('loc-10', 10), 10);
        await repository.save(session1);

        const session2 = ReaderSession.start('session-2', 'reader-1', 'book-2', createPos('loc-0', 0));

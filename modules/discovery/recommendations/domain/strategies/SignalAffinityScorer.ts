...
export class SignalAffinityScorer implements RecommendationScorer {
  score(
    ...
    const interaction = context.interactions?.find(
      (i) => i.bookId === candidate.bookId,
    );
    const feature = context.bookFeatures?.find(
      (f) => f.bookId === candidate.bookId,
    );
    ...
    if (interaction) {
      ...
      if (interaction.lastActivityAt) {
        const daysSinceActivity =
          (Date.now() - interaction.lastActivityAt.getTime()) /
          (1000 * 60 * 60 * 24);
        // 30 day half-life

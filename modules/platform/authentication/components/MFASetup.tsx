...
import { getMFAStatus, enrollMFA, verifyMFA, disableMFA } from '@/modules/platform/authentication/actions/auth';
...
export default function MFASetup() {
    ...
    const checkMFAStatus = async () => {
        ...
        if (res.success && res.data?.isEnabled) {
            setIsEnabled(true);
    ...
    const startEnrollment = async () => {
        ...
        try {
            ...
            // Generate QR Code
            const qrUrl = await QRCodeLib.toDataURL(res.data.uri);
            setQrCodeUrl(qrUrl);
            setStep('qr');
        } catch (error: any) {
            console.error(error);

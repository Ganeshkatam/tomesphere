...
export class StartReadingSessionHandler {
    ...
    async execute(command: StartReadingSessionCommand): Promise<ActionResult<StartReadingSessionOutput>> {
        try {
            const { sessionId, readerId, bookId, location, chapter, page, progress } = command.input;
            ...
            const session = ReaderSession.start(sessionId, readerId, bookId, position);
            ...
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error starting session',
            };
        }
    }
}

...
2. **Persistence Isolation**: Repositories only handle persistence. A dedicated `DomainEventPublisher` application service collects, dispatches, and clears events from the aggregate *only* after a successful database transaction commit.

...
import { loginWithPassword, sendMagicLinkServer, verifyMagicLinkServer, getMFAStatus, verifyMFA } from '@/modules/platform/authentication/actions/auth';
import { showError, showSuccess } from '@/lib/toast';
import { Lock, Mail, Smartphone, ArrowRight, Globe, ChevronDown, Sparkles, BookOpen, Check, Shield, RefreshCw, Eye, EyeOff, PenTool, CheckSquare, FileText } from 'lucide-react';
...
/* ──────────────────────────────────────────
   Shared input class
────────────────────────────────────────── */
const inputCls = 'w-full bg-[var(--surface-raised)] border border-[var(--border-default)] text-slate-50 placeholder-slate-500 rounded-xl px-4 py-3.5 pl-12 focus:outline-none focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/40 transition-all text-sm';
const btnCls = 'w-full h-[54px] rounded-xl font-semibold text-sm flex items-center justify-center gap-2 group transition-all duration-200 disabled:opacity-50 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white shadow-[0_4px_20px_rgba(99,102,241,0.25)] active:scale-[0.99]';
...
export default function EnhancedLoginPage() {
    ...
    const detectInputType = (value: string): 'email' | 'phone' | 'unknown' => {
        ...
        if (/^\d{10,15}$/.test(value) || /^\+\d{10,15}$/.test(value)) return 'phone';
        return 'unknown';
    };
    ...
    const checkMFA = async () => {
        try {
            ...
            if (res.success && res.data.isEnabled) { setStep('mfa'); return true; }
            return false;
        } catch { return false; }
    };
    ...
    const handleContinue = async (e: React.FormEvent) => {
        e.preventDefault();
        if (detectInputType(input) === 'unknown') { showError('Please enter a valid email or phone number'); return; }
        if (authMode === 'magic') { await handleMagicLinkLogin(e); } else { setStep('password'); }
    };
    ...
    const handleMagicLinkLogin = async (e: React.FormEvent) => {
        ...
        try {
            let id = input;
            if (isPhone) { const c = input.replace(/[\s-]/g, ''); id = c.startsWith('+') ? c : `${countryCode}${c}`; }
            const res = await sendMagicLinkServer(id, isPhone);
            if (!res.success) throw new Error(res.error);
            showSuccess(isPhone ? 'SMS code sent!' : 'Magic code sent to your email!');
            setStep('otp');
        } catch (err: any) { showError(err.message || 'Failed to send verification code'); }
    ...
    const handlePasswordLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (mfaRequired) { setLoading(false); return; }
            showSuccess('Logged in successfully!');
            router.push('/home');
        } catch (err: any) { showError(err.message || 'Invalid password'); setLoading(false); }
    };

    const handleMFAVerify = async (e: React.FormEvent) => {
        e.preventDefault(); setLoading(true);
        try {
            throw new Error('MFA is currently being migrated to Server Actions. Please contact support.');
            showSuccess('MFA Verified! Logging in...'); router.push('/home');
        } catch (err: any) { showError(err.message || 'Invalid code'); }
        finally { setLoading(false); }
    };
    ...
    const handleVerifyOtp = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.success) throw new Error(res.error);
            showSuccess('Verified! Logging in...'); router.push('/home');
        } catch (err: any) { showError(err.message || 'Invalid code'); }
        finally { setLoading(false); }
    };
            ...
            <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
                ...
                <div className="relative w-full max-w-sm bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-2xl font-bold text-white mb-1">Two-Factor Auth</h1>
        ...
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 sm:p-8 relative">
            ...
            <div className="w-full max-w-6xl bg-[var(--surface-default)]/85 backdrop-blur-2xl border border-[var(--border-subtle)] rounded-[24px] overflow-hidden flex flex-col lg:flex-row shadow-[var(--shadow-card)] relative z-10">
                ...
                <div className="w-full lg:w-[65%] bg-gradient-to-b from-[var(--surface-raised)]/60 via-[var(--surface-default)]/50 to-[var(--surface-raised)]/70 relative p-8 sm:p-10 lg:p-10 flex flex-col justify-between gap-8 overflow-hidden">
                    ...
                    {/* Logo */}
                    <a href="/" className="flex items-center gap-3 group w-fit relative z-10">
                        <span className="text-3xl group-hover:scale-105 transition-transform">📚</span>
                        <div>
                            <span className="text-lg font-display font-bold bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent">TomeSphere</span>
                            <p className="text-[9px] font-semibold text-slate-500 uppercase tracking-[0.2em] mt-2">READ • LEARN • REMEMBER</p>
                        </div>
                    </a>
                    ...
                    <div className="space-y-6 relative z-10">
                        <div className="space-y-3">
                            ...
                            <p className="text-slate-400 text-sm leading-relaxed max-w-sm">
                                Access your library, notes, highlights, and study plans in one unified space.
                            </p>
                        </div>
                            ...
                            <div className="glass bg-[var(--surface-default)]/60 border border-[var(--border-subtle)] rounded-2xl p-5 w-full max-w-sm sm:w-[350px] space-y-4 shadow-2xl relative overflow-hidden">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h4 className="text-slate-50 text-xs font-semibold tracking-wide truncate max-w-[200px]">Algorithms</h4>
                                    </div>
                                    <span className="text-[10px] text-slate-500 font-mono animate-pulse">Page 143</span>
                                </div>
                                ...
                                <div className="border-t border-white/[0.04] pt-3 space-y-3">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-[11px] text-[var(--text-secondary)] italic pl-3 border-l border-yellow-500/40 leading-relaxed">
                                            &quot;Graph traversal algorithms like DFS and BFS form the foundation...&quot;
                                        </p>
                                    </div>
                ...
                <div className="w-full lg:w-[35%] p-8 sm:p-14 lg:p-16 bg-[var(--surface-raised)]/20 flex flex-col justify-between relative z-10">
                            ...
                            <form onSubmit={handleContinue} className="space-y-4">
                                <div className="relative group">
                                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10">
                                        {isPhone
                                            ? <Smartphone className="text-slate-500 group-focus-within:text-indigo-400 transition-colors" size={16} />
                                            : <Mail className="text-slate-500 group-focus-within:text-indigo-400 transition-colors" size={16} />
                                        }
                                    </div>
                                    <div className="flex">
                                        {isPhone && (
                                                    ...
                                                    <div className="absolute top-full left-0 mt-1 w-60 bg-slate-900 border border-white/10 rounded-xl shadow-xl z-50 max-h-56 overflow-y-auto">
                                                        {COUNTRY_CODES.map((c) => (
                                                            <button key={c.code} type="button"
                                                                ...
                                                                <span className="ml-auto text-slate-500 text-xs">{c.code}</span>
                                                            </button>
                                                        ))}
                                                    </div>
                                                )}
                                ...
                                <button type="submit" disabled={loading || !input}
                                    className={btnCls}>
                                    {loading ? <Spinner /> : <>{authMode === 'password' ? 'Continue' : 'Send Magic Link'} <ArrowRight size={15} className="group-hover:translate-x-0.5 transition-transform" /></>}
                                </button>

                                <div className="relative my-4">
                                    <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[var(--border-default)]" /></div>
                                    <div className="relative flex justify-center text-xs"><span className="px-3 bg-[var(--surface-default)] text-slate-500">or</span></div>
                                </div>
                            ...
                            <form onSubmit={handlePasswordLogin} className="space-y-4">
                                <div className="flex items-center justify-between px-4 py-3 bg-[var(--surface-raised)] border border-[var(--border-default)] rounded-xl">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-[10px] text-slate-500 uppercase tracking-wide">Signing in as</p>
                                            <p className="text-slate-50 text-sm font-medium">{input}</p>
                                        </div>
                                    </div>
                                    <button type="button" onClick={() => { setStep('input'); setPassword(''); }}
                                ...
                                <div className="relative group">
                                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                        <Lock className="text-slate-500 group-focus-within:text-indigo-400 transition-colors" size={16} />
                                    </div>
                                    <input
                                ...
                                <div className="flex justify-end">
                                    <a href="/forgot-password" className="text-xs text-slate-500 hover:text-slate-50 transition-colors">
                                        Forgot password?
                                    </a>
                                </div>
                                ...
                                <button type="submit" disabled={loading || !password}
                                    className={btnCls}>
                                    {loading ? <Spinner /> : <>Resume Reading <ArrowRight size={15} /></>}
                                </button>
                            </form>
                        )}
                        ...
                        {step === 'otp' && (
                            <form onSubmit={handleVerifyOtp} className="space-y-5">
                                <div className="text-center">
                                    ...
                                    <h3 className="text-lg font-bold text-slate-50 mb-1">Check your inbox</h3>
                                    <p className="text-slate-400 text-sm">
                                        Magic code sent to <span className="text-white font-medium">{input}</span>
                                    </p>
                                </div>
                    ...
                    <div className="space-y-5 mt-8 border-t border-white/5 pt-6 text-center">
                        <div className="text-slate-500 text-xs">
                            New to TomeSphere?{' '}
                            <button onClick={() => router.push('/signup')} className="text-indigo-400 hover:text-indigo-300 font-semibold transition-colors">

...
export function runLibraryRepositoryContract() {
  console.log(
    "LibraryRepository contract tests pending testing framework installation",
  );
  ...
  try {
    ...
    console.error(
      "FAILED: Valid transition from want_to_read to currently_reading failed",
    );
  }
  ...
  try {
    ...
    console.error(
      "FAILED: Finishing a book did not update progress and timeline correctly",
    );
  }
  ...
  try {
    ...
    console.error(
      "FAILED: Restoring to want_to_read failed to clear finished state: ",
      error,
    );
  }

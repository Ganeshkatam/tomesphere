...
export function registerRecommendationSignalHandlers() {
    eventBus.subscribe('reader:page_completed', async (event: any) => {
        try {
            ...
            await supabase.rpc('refresh_recommendation_signals', { target_user_id: event.userId });
        } catch (e) {
            console.error('Failed to update recommendation signals on page completion', e);
        }
    });

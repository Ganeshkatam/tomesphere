...
interface AnnotationState {
    ...
    addAnnotation: (payload: AnnotationCreatePayload, bookId: string, userId: string) => void;
...
export const useAnnotationStore = create<AnnotationState>((set) => ({
    ...
    setAnnotations: (annotationsList) => {
        ...
        annotationsList.forEach(a => { map[a.id] = a; });
        set({ annotations: map });
    },

    addAnnotation: (payload, bookId, userId) => set((state) => {
        ...
        return {
            annotations: {
                ...
                [id]: {
                    ...
                    ...updates,
                    updatedAt: new Date().toISOString(),
                    syncState: existing.syncState === 'synced' ? 'pending_update' : existing.syncState
                }
            }
        };
    }),

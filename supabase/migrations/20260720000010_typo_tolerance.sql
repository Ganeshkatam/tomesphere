-- Migration: Typo Tolerance & Synonym Expansion Integration
-- Implements FTS fallback using pg_trgm and integrates normalize_search_query
...
BEGIN
    ...
    ELSE
        v_fts_count := 1; -- Bypass if no query
    END IF;

    -- 2. Execute FTS Strategy
    IF v_fts_count > 0 OR v_tsquery IS NULL THEN
        RETURN QUERY
        WITH filtered_docs AS (
            SELECT 
                d.book_id, d.slug, d.title, d.subtitle, d.authors, d.genres, d.subjects,
                d.language, d.average_rating, d.rating_count, d.popularity_score,
        ...
        scored_docs AS (
            SELECT *, (base_rank + (popularity_score * 0.15))::NUMERIC AS computed_relevance
            FROM filtered_docs
        )
        SELECT 
            s.book_id::UUID, s.slug, s.title, s.subtitle, s.authors, s.genres, s.subjects,
            s.language, s.average_rating, s.rating_count, s.popularity_score::NUMERIC,
    ...
    -- 3. Execute Fallback Strategy (pg_trgm)
    ELSE
        RETURN QUERY
        WITH filtered_docs AS (
            SELECT 
                ...
                d.language, d.average_rating, d.rating_count, d.popularity_score,
                d.is_public, d.publication_year,
                -- Use pg_trgm strict_word_similarity against title and primary author
                GREATEST(

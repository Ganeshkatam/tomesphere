...
export class LibraryMapper {
    static toDomain(row: LibraryBookRow, isFavorite: boolean = false): LibraryBook {
    ...
    static toPersistence(entity: LibraryBook): LibraryBookRow {
        return {
            ...
            status: (entity.state.value === 'abandoned' ? 'finished' : entity.state.value) as Tables<'library_books'>['status'], // Map abandoned to finished for DB

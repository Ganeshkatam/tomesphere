...
export default function LandingHero({
  ...
  return (
    <section className={`relative pt-12 pb-12 sm:pt-16 sm:pb-16 lg:pt-20 lg:pb-20 overflow-hidden min-h-[75vh] flex items-center justify-center ${isSticky ? 'z-40' : 'z-10'}`}>
      ...
      <div className="w-full max-w-[2000px] mx-auto px-2 sm:px-4 lg:px-6 relative z-20">
        <FadeIn className="text-center w-full mx-auto relative" delay={0.2}>
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[var(--border-default)] bg-[var(--surface-raised)] backdrop-blur-md mb-8 hover:bg-[var(--surface-overlay)] transition-colors cursor-default">
            <span className="flex h-2 w-2 rounded-full bg-accent animate-pulse" />
            <span className="text-sm font-medium text-[var(--text-secondary)]">Your Reading Companion</span>
          </div>

          <h1 className="text-5xl sm:text-7xl lg:text-8xl font-display font-bold mb-8 leading-[1.1] tracking-tight">
            <span className="text-balance text-[var(--text-primary)] drop-shadow-sm">Find Books.</span>
            <br />
          ...
          <p className="text-lg sm:text-2xl mb-12 text-balance max-w-3xl mx-auto leading-relaxed text-[var(--text-secondary)]">
            Discover books you&apos;ll love, read with focus, highlight what matters, and remember it for longer.
          </p>

          {/* Central Search Bar - Premium Glass / Sticky Morph */}
          <div className={`w-full transition-all duration-300 ${isSticky ? 'fixed top-6 left-1/2 -translate-x-1/2 max-w-3xl w-[92%] z-40 px-4 animate-in fade-in slide-in-from-top-6 duration-300' : 'relative max-w-3xl mx-auto mb-4 px-4 sm:px-0'}`}>
            <SlideUp className="w-full" delay={0.4}>
              <div className="relative group">
                <div className={`absolute -inset-1 bg-gradient-to-r from-primary to-secondary rounded-3xl opacity-25 group-hover:opacity-50 blur transition duration-1000 group-hover:duration-200 ${isSticky ? 'rounded-2xl' : 'rounded-3xl'}`} />

                <div className={`relative flex flex-row items-center transition-all duration-300 ${isSticky ? 'gap-3 p-2 rounded-2xl bg-[var(--surface-default)]/95 backdrop-blur-3xl border border-[var(--primary)]/60 shadow-[0_0_50px_rgba(99,102,241,0.25)]' : 'gap-2 sm:gap-4 p-1.5 sm:p-3 rounded-2xl bg-[var(--surface-default)] border border-[var(--border-default)] shadow-2xl'} focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/50`}>
                  {/* Search Input Container */}
                  <div id="hero-search" className={`relative flex-1 group/input flex items-center transition-all duration-300 ${isSticky ? 'h-11 sm:h-12' : 'h-12 sm:h-14'}`}>
                    <div className="absolute inset-y-0 left-2 sm:left-4 flex items-center pointer-events-none">
                      <span className="text-xl sm:text-2xl opacity-50 text-slate-400 group-focus-within/input:text-primary transition-colors">🔍</span>
                    </div>
                    <input
                      type="text"
                      value={searchQuery}
          ...
          <SlideUp className="w-full max-w-3xl mx-auto mt-6 px-4 sm:px-0 flex flex-wrap gap-2 justify-center text-xs text-slate-400" delay={0.5}>
            <span className="font-semibold text-slate-500">Popular:</span>
            {['Computer Science', 'Algorithms', 'Quantum Physics', 'Socrates', 'Data structures'].map((topic) => (
              <button
                key={topic}
                onClick={() => {

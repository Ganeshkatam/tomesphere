...
import { EventRegistry, EventClass } from '../../application/events/EventRegistry';
...
export class InProcessEventBus implements EventBus, DomainEventPublisher {
    ...
    async publish(
        ...
        if (eventsOrAggregate instanceof AggregateRoot) {
            ...
            try {
                ...
                console.error(`[InProcessEventBus] DomainEventPublisher failed. Aggregate events NOT cleared.`, error);
                throw error;
            }
        } else {
    ...
    private async publishEvents(events: readonly DomainEvent[], metadata?: EventMetadata): Promise<void> {

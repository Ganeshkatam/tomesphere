    ...
    async search(query: SearchQuery): Promise<{ documents: BookSearchDocument[], totalCount: number }> {
        ...
        if (query.text) {
            ...
            results = results.filter(doc => 
                ...
                (doc.description && doc.description.toLowerCase().includes(lowerText)) ||
        ...
        if (query.filters.categories && query.filters.categories.length > 0) {
            results = results.filter(doc => 
                query.filters.categories!.some((c: string) => doc.categories.includes(c))
            );
        }
        if (query.filters.language) {
            results = results.filter(doc => doc.language === query.filters.language);
        }
        ...
        // Pagination
        const paginated = results.slice(query.pagination.offset, query.pagination.offset + query.pagination.limit);

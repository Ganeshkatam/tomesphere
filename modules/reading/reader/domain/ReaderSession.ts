...
export class ReaderSession {
  ...
  get highlights(): HighlightCollection {
  ...
  get totalDurationSeconds(): number {
    return this.props.totalDurationSeconds;
  }
  ...
  public updatePosition(newPosition: ReadingPosition): void {
    ...
    // Accumulate duration
    const now = new Date();
    const durationSinceResume = Math.floor(
      (now.getTime() - this.props.lastResumedAt.getTime()) / 1000,
    );
  ...
  public updateHighlightNote(highlightId: string, newNote: string): void {
    ...
    const highlight = Array.from(this.props.highlights).find(
      (h) => h.id === highlightId,
    );

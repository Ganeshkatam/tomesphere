...
class MockSearchRepository implements SearchRepository {
    ...
    async search(query: any) { return { documents: [], totalCount: 0 }; }
    async index(doc: any) { this.indexedDocs.push(doc); }
...
class BookAggregate extends AggregateRoot<any> {
    ...
    publish(title: string, authors: string[]): void {
        this.addDomainEvent(new BookPublishedEvent(
            this.id,
            1, // aggregateVersion
            title,
            authors,
...
class ReaderSessionAggregate extends AggregateRoot<any> {
    ...
    completeBook(bookId: string): void {
        this.addDomainEvent(new ReadingCompletedEvent(
...
describe('E2E Event-Driven Monolith Flow', () => {
    it('propagates events reactively and updates projections across modules', async () => {
        ...
        expect(searchRepo.indexedDocs[0].title).toBe('Refactoring Bounded Contexts');

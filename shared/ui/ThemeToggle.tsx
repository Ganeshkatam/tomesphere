...
export default function ThemeToggle() {
  ...
  const state = {
    system: {
      icon: (
        <Monitor
          size={18}
          className="text-slate-400 transition-transform group-hover:scale-110 duration-300"
        />
      ),
    ...
    light: {
      icon: (
        <Sun
          size={18}
          className="text-amber-500 transition-transform group-hover:rotate-90 duration-500"
        />
      ),
    ...
    dark: {
      icon: (
        <Moon
          size={18}
          className="text-indigo-400 transition-transform group-hover:-rotate-12 duration-500"
        />
      ),
  ...
  return (
    ...
    >
      ...
      {/* Glow effect */}
      <div
        className={`absolute inset-0 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${state.glowClass}`}
      />
    </button>
  );
}

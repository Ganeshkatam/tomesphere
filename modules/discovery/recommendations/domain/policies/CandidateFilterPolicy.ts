...
export interface CandidateFilterPolicy {
    filter(candidates: readonly RecommendationCandidate[], context: RecommendationContext): RecommendationCandidate[];
}
...
export class DefaultCandidateFilterPolicy implements CandidateFilterPolicy {
    filter(candidates: readonly RecommendationCandidate[], context: RecommendationContext): RecommendationCandidate[] {
        ...
        return candidates.filter(c => {
            ...
            if (preferredLangs.size > 0 && c.language && !preferredLangs.has(c.language)) {
                return false;
            }

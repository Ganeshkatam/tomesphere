...
export default function PhoneLoginPage() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const error = !res.ok ? new Error(data.error?.message || "Failed to send OTP") : null;
      ...
      showError(
        error.message || "Failed to send OTP. Check Supabase phone auth setup.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      const error = !res.ok ? new Error(data.error?.message || "Invalid OTP") : null;
      ...
      <div className="w-full max-w-md">
        ...
        <div className="glass rounded-xl p-4 mt-6">
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
            <strong>Note:</strong> Phone authentication must be enabled in
            Supabase Dashboard. If OTP doesn&apos;t arrive, check your SMS

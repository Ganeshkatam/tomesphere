...
const ReadingGoalProgress = dynamic(() => import('@/modules/progress/analytics/components/ReadingGoalProgress'), {
    loading: () => <div className="h-32 bg-white/5 animate-pulse rounded-xl" />
});
const RecentlyViewed = dynamic(() => import('@/modules/reading/books/components/RecentlyViewed'), {
    loading: () => <div className="h-32 bg-white/5 animate-pulse rounded-xl" />
});
const StudyPlanWidget = dynamic(() => import('@/modules/planner/planner/components/StudyPlanWidget'), {
    ...
    return (
        <aside className="space-y-6 w-full lg:w-80 flex-shrink-0">
            ...
            <div className="bg-slate-900/50 rounded-xl p-6 border border-white/5">
                ...
                {today && (today.pages_read > 0 || today.reading_time_minutes > 0 || today.books_completed > 0) ? (
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between p-3 rounded-lg bg-white/5">
                            ...
                            <span className="font-bold text-white">{today.reading_time_minutes} min</span>
                        </div>
                        {today.books_completed > 0 && (
                            <div className="flex items-center justify-between p-3 rounded-lg bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/20">
                                ...
                                <span className="font-bold text-amber-300">{today.books_completed}</span>
                            </div>
                        )}
                    </div>
                ) : (
            ...
            <div className="bg-slate-900/50 rounded-xl p-6 border border-white/5">
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-indigo-400">{userStats?.totalLikes || 0}</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-slate-400">Favorite Genre</span>
                        <span className="font-bold text-purple-400">{userStats?.favoriteGenre || 'None'}</span>
                    </div>
                </div>
            </div>
        </aside>

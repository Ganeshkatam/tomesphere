...
export class GetPersonalizedRecommendationsHandler {
    ...
    async execute(query: GetPersonalizedRecommendationsQuery): Promise<ActionResult<RecommendationReadModel[]>> {
        try {
            ...
            const rankedRecommendations = await pipeline.execute(plan, context, execContext);

            // 5. Format results with explanation service and map to Read Model
            const limit = query.limit || 20;
            const results: RecommendationReadModel[] = rankedRecommendations.slice(0, limit).map(r => ({

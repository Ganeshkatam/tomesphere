...
export class ProgressMapper {
  static toDomain(
    ...
    const readingGoal = ReadingGoal.create(
      dailyTarget,
      yearlyTarget,
      0,
      0,
    ...
    const readingStreak = ReadingStreak.create(
      ...
      statsRow?.last_activity_at
        ? statsRow.last_activity_at.split("T")[0]
        : null,
    );

    const experiencePoints = ExperiencePoints.create(
      statsRow?.total_points ?? 0,
    );
    ...
    return UserProgress.fromPersistence(
      ...
      achievements,
      statsRow?.last_activity_at
        ? new Date(statsRow.last_activity_at)
        : new Date(),
    );
  }
}

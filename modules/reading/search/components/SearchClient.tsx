...
// ─── Constants ───────────────────────────────────────────────
const RECENT_SEARCHES_KEY = 'tomesphere_recent_searches';
const MAX_RECENT = 5;
                            ...
                            {showRecent && !query && recentSearches.length > 0 && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
                                    <div className="flex items-center justify-between px-4 pt-3 pb-2">
                                        <span className="text-xs text-slate-500 font-medium uppercase tracking-wider">Recent Searches</span>
                                        <button
                ...
                <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
                    <h1 className="text-2xl font-bold text-white flex items-center gap-3">
                        <span>
                            {books.length > 0 ? (
                                <>
                                    Found <span className="text-primary">{totalCount}</span> results
                                ...
                                <>Showing <span className="text-primary">{totalCount}</span> books</>
                            )}
                        </span>
                    </h1>
                    {totalCount > 0 && (
                    ...
                    <FadeIn>
                        <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/5">
                            ...
                            <h3 className="text-2xl font-bold text-white mb-3">
                                {initialQuery
                                    ? <>No books matching &quot;{highlightMatch(initialQuery, initialQuery)}&quot;</>
                                    : 'Start searching'}
                            </h3>
                            ...
                            {initialQuery && (
                                <div className="space-y-3">
                                    ...
                                    {recentSearches.length > 0 && (
                                        <div className="mt-6">
                                            <p className="text-xs text-slate-500 uppercase tracking-wider mb-3">Try a recent search</p>
                                            <div className="flex flex-wrap gap-2 justify-center">
                                                {recentSearches.filter(s => s !== initialQuery).slice(0, 3).map((term, idx) => (
                                                    <button
                                                        key={idx}

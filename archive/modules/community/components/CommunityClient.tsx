...
import { supabase } from '@/modules/shared/core/database/client'; // keeping for realtime only
import { getDiscussions, getBookClubs, toggleDiscussionLike, toggleClubMembership } from '@/modules/community/actions/community';
import { showError, showSuccess } from '@/lib/toast';
import { MessageSquare, Heart, Users, Plus, TrendingUp, Clock, BookOpen } from 'lucide-react';
import CreateDiscussionModal from '@/modules/community/components/CreateDiscussionModal';
import CreateClubModal from '@/modules/community/components/CreateClubModal';
...
interface CommunityClientProps {
    ...
    initialClubs: BookClub[];
}

export default function CommunityClient({ user, initialDiscussions, initialClubs }: CommunityClientProps) {
    const router = useRouter();
    const [activeTab, setActiveTab] = useState<'discussions' | 'clubs' | 'trending'>('discussions');
    const [discussions, setDiscussions] = useState<Discussion[]>(initialDiscussions);
    const [bookClubs, setBookClubs] = useState<BookClub[]>(initialClubs);
    const [showCreateModal, setShowCreateModal] = useState(false);

    useEffect(() => {
        // Switch tabs logic if needed: fetch data dynamically if switching to clubs
        if (activeTab === 'discussions' && discussions.length === 0) {
            loadDiscussions();
        } else if (activeTab === 'clubs' && bookClubs.length === 0) {
            loadBookClubs();
        }
    ...
    const loadDiscussions = async () => {
        ...
        if (res.success) {
            setDiscussions(res.data as any);
        }
    };
    ...
    const handleLikeDiscussion = async (discussionId: string, isLiked: boolean) => {
        ...
        try {
            const res = await toggleDiscussionLike(discussionId);
            if (res.success) {

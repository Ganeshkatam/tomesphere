import { EventPayloads, PlatformEventName } from '@/modules/shared/core/events/types';
...
export class AnalyticsEventHandlers {
    ...
    async handleReaderProgressUpdated(payload: EventPayloads['reader:progress_updated']): Promise<void> {
    ...
    async handleReaderBookCompleted(payload: EventPayloads['reader:book_completed']): Promise<void> {
        const { userId, bookId } = payload;
        const dateStr = new Date().toISOString().split('T')[0];
        await this.store.recordBookCompleted(userId, bookId, dateStr);
    }

    async handleLibraryBookAdded(payload: EventPayloads['library:book_added']): Promise<void> {

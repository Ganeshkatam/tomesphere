    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-slate-400 mt-1">Manage and track your reading lifecycle.</p>
            </div>
            ...
            <div className="space-y-3">
                {filteredItems.length > 0 ? (
                        ...
                        >
                            <div className="flex items-center gap-4 min-w-0">
                                <img
                                    ...
                                    alt={item.book.title}
                                    className="w-12 h-16 object-cover rounded shadow-sm border border-[var(--border-subtle)] shrink-0"
                                />
                                <div className="min-w-0">
                                    <h3 className="text-sm sm:text-base font-bold text-slate-50 truncate">{item.book.title}</h3>
                                    <p className="text-xs text-slate-400 font-semibold truncate mt-0.5">{item.book.author}</p>
                    ...
                    <div className="text-center py-12 rounded-xl border border-dashed border-[var(--border-default)]">
                        <p className="text-sm text-slate-400 font-medium">No books matches the filter.</p>
                        <Link href="/discover" className="inline-block mt-2 text-xs font-bold text-indigo-400 hover:underline">

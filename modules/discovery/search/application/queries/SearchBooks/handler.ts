...
import { RankingPolicy, RankableDocument } from '../../../domain/policies/RankingPolicy';
...
export class SearchBooksHandler {
    ...
    async execute(query: SearchBooksQuery): Promise<ActionResult<SearchResponseReadModel>> {
        try {
            const searchQuery = SearchQuery.create(query.request);
            
            // Infrastructure executes the raw search
            const { documents, totalCount } = await this.searchRepository.search(searchQuery);
            ...
            const rankableDocs: RankableDocument[] = documents.map(doc => ({
                ...
                publishDate: doc.publicationYear ? new Date(doc.publicationYear, 0) : undefined,
            ...
            const results = scoredDocs.map(r => {
                ...
                return {
                    ...
                    descriptionSnippet: doc.description?.substring(0, 150) + (doc.description && doc.description.length > 150 ? '...' : ''),
                    score: r.score,
                };
            });
            ...
            return {
                ...
                data: {
                    results,
                    totalCount,
                    hasMore: totalCount > searchQuery.pagination.offset + searchQuery.pagination.limit
                }
            };
        } catch (error) {
            return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error performing search',
            };
        }
    }
}

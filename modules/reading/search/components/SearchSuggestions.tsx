...
export default function SearchSuggestions({ query, onSelect, className = '', localBooks }: SearchSuggestionsProps) {
    ...
    useEffect(() => {
        ...
        if (fuseIndex) {
            ...
            for (const result of results) {
                ...
                newSuggestions.push({
                    ...
                    text: book.title,
                    id: book.id,
                    subtitle: `by ${book.author || 'Unknown'}`,
                    score: result.score,
                });

                // Deduplicated author suggestions
                if (book.author && !seenAuthors.has(book.author.toLowerCase())) {
        ...
        const timeoutId = setTimeout(async () => {
            try {
                const { getSearchSuggestions } = await import('@/modules/reading/books/actions/books');
                ...
                res.data.forEach((book: any) => {
                    ...
                    if (book.author?.toLowerCase().includes(query.toLowerCase())) {
                        if (!seenAuthors.has(book.author.toLowerCase())) {
                            seenAuthors.add(book.author.toLowerCase());
                            newSuggestions.push({ type: 'author', text: book.author, subtitle: 'Author' });
                        }
                    }
                });
                    ...
                    {suggestions.map((item, idx) => (
                        <li key={`${item.type}-${item.text}-${idx}`}>
                            ...
                            >
                                ...
                                {item.type === 'book' && (
                                    <span className="text-[10px] text-slate-600 uppercase tracking-wider font-medium">↵</span>
                                )}
                            </button>
                        </li>
                    ))}

...
export class UserProgress extends AggregateRoot<UserProgressProps> {
    ...
    get readingGoal(): ReadingGoal { return this.props.readingGoal; }
    get readingStreak(): ReadingStreak { return this.props.readingStreak; }
    get experiencePoints(): ExperiencePoints { return this.props.experiencePoints; }
    get achievements(): AchievementCollection { return this.props.achievements; }
    get updatedAt(): Date { return this.props.updatedAt; }
    get level() { return LevelPolicy.calculateLevel(this.props.experiencePoints); }
    ...
    applyReadingActivity(activity: ReadingActivity) {
        ...
        const { newStreak, extended, lost } = this.props.readingStreak.evaluate(activity.date);
        ...
        if (extended) {
            this.addDomainEvent(new StreakExtended(this.id, newStreak.currentStreakDays));
        }
        ...
        // 3. Update Goals
        const { newGoal, justCompletedDaily } = this.props.readingGoal.applyProgress(activity);
        this.props.readingGoal = newGoal;

        if (justCompletedDaily) {
            this.addDomainEvent(new ReadingGoalCompleted(this.id, 'daily', newGoal.dailyMinutesTarget));
        }

        this.props.updatedAt = new Date();
        this.addDomainEvent(new ReadingProgressApplied(
            this.id, 
            activity.minutes, 
            activity.pages, 
    ...
    unlockAchievement(achievementId: string, rarity: string | null = null, source: string | null = null) {
        const achievement = Achievement.create(crypto.randomUUID(), achievementId, new Date(), rarity, source);
        const { collection, newlyUnlocked } = this.props.achievements.unlock(achievement);
    ...
    updateReadingGoal(dailyMinutes: number, yearlyBooks: number) {
        ...
        this.props.updatedAt = new Date();
        this.addDomainEvent(new ReadingGoalUpdated(this.id, dailyMinutes, yearlyBooks));
    }
}

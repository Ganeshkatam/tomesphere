...
import { User, LogOut, Menu, X, Library, Target, Search } from 'lucide-react';
import { getCurrentUser, logOut } from '@/modules/platform/authentication/actions/auth';
import { showError, showSuccess } from '@/lib/toast';
import { useEffect, useState } from 'react';
import NotificationCenter from '@/modules/user/profile/presentation/components/NotificationCenter';
...
export default function Navbar({ role, currentPage, user: propUser, minimal = false }: NavbarProps) {
    ...
    useEffect(() => {
        ...
        // Get initial session client-side
            ...
            <nav className="glass-nav sticky top-0 z-50 h-[76px] flex items-center">
                <div className="w-full px-4 sm:px-8 lg:px-16">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-8">
                            ...
                            <a href={user ? "/home" : "/"} className="flex items-center gap-3 group">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-lg font-extrabold tracking-tight text-[var(--text-primary)] leading-none">TomeSphere</span>
                                    <span className="text-[9px] text-[var(--text-tertiary)] font-bold tracking-wider mt-1 block uppercase">Built for learners</span>
                                </div>
                            </a>
                            ...
                            {!minimal && user && (
                                <div className="hidden xl:flex items-center gap-6 border-l border-[var(--border-default)] pl-8 h-8">
                                    {navLinks.filter(link => link.label !== 'Profile').map((link) => {
                                        ...
                                        return (
                                            <a
                                                key={link.href}
                                                href={link.href}
                                                className={`relative py-2 text-sm font-medium transition-all duration-200 group flex items-center gap-2 ${active ? 'text-[var(--text-primary)] font-semibold active-link' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'
                                                    }`}
                                            >
                                                <Icon size={14} className={active ? link.color : 'text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]'} />
                                                <span>{link.label}</span>
                                                <span className={`absolute bottom-[-10px] left-0 w-full h-[2px] bg-indigo-500 transition-transform duration-200 origin-left ${active ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'}`} />
                                            </a>
                                        );
                                    })}
                                </div>
                        ...
                        {!minimal && user && (
                            <form onSubmit={handleSearchSubmit} className="hidden md:flex items-center relative max-w-sm lg:max-w-md w-full mx-8">
                        ...
                        <div className="flex items-center gap-4">
                            {user ? (
                                <>
                                    {/* Notifications Center with custom unread purple dot state */}
                                                ...
                                                <div className="absolute right-0 mt-3 w-80 profile-dropdown-card bg-[var(--surface-floating)] border border-[var(--border-default)] rounded-2xl shadow-[var(--shadow-dropdown)] z-50 p-5 space-y-4">
                                                    ...
                                                    <div className="flex items-center gap-3 pb-3 border-b border-[var(--border-subtle)]">
                                                        ...
                                                        <div className="min-w-0">
                                                            ...
                                                            <p className="text-[10px] text-[var(--text-tertiary)] font-medium truncate">{user?.email}</p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center justify-between px-3 py-2 border border-[var(--border-subtle)] rounded-xl bg-[var(--surface-default)]">
                                                        <span className="text-[var(--text-secondary)] font-semibold text-xs">Theme Mode</span>
                                                        <ThemeToggle />
                                                    </div>
                        ...
                        {user ? (
                                ...
                                {navLinks.map((link) => {
                                    ...
                                    return (
                                        <a
                                            ...
                                            href={link.href}
                                            onClick={() => setMobileMenuOpen(false)}
                                            className={`flex items-center gap-3 px-6 py-4 transition-all ${active
                                                ? 'bg-white/5 text-white border-r-2 border-indigo-500'
                                                : 'text-slate-300 hover:bg-white/5'
                                                }`}
                                        >
                                            <Icon size={20} className={active ? link.color : 'text-slate-400'} />
                            ...
                            <>
                                ...
                                >
                                    ...
                                    <span className="font-medium">Discover Books</span>
                                </a>
                                <a
                                    href="/login"
                                    onClick={() => setMobileMenuOpen(false)}

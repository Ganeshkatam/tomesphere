...
export class EpubJsRenderer implements ReaderRenderer {
    ...
    private selectionListeners: Set<(anchor: SelectionAnchor, text: string) => void> = new Set();
    ...
    async addHighlight(highlight: ReaderHighlight): Promise<void> {
        ...
        this.rendition.annotations.highlight(cfiRange, { fill: highlight.color }, (e: Event) => {
            // Prevent event from propagating and triggering other clicks (e.g. page turns)
            e.preventDefault();
            e.stopPropagation();
            this.highlightClickListeners.forEach(listener => listener(highlight.id));
        }, 'highlight', { 'data-highlight-id': highlight.id });
    }
    ...
    onTextSelected(callback: (anchor: SelectionAnchor, text: string) => void): () => void {

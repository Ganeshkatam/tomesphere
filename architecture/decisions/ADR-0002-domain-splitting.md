...
_Note: In the previous state, all of these sub-domains were treated as a single massive 'learning' bounded context, creating cyclic dependencies when analytics needed to track progress from learning features._
...
_Note: Domains (reading, learning, progress, planner) are now strictly forbidden from importing each other (preventing circular dependencies). Coordination happens solely in the integration layer (`me` or application orchestrators) or via `shared` interfaces._

import type { 
    ...
    SearchResult, 
    DocumentMetadata 
} from '@/modules/reading/reader/contracts/DocumentEngine';
...
export class PdfJsEngine implements DocumentEngine {
    ...
    async renderPage(pageNumber: number, scale: number = 1.0): Promise<RenderedPage> {
        if (!this.pdfDocument) throw new Error('Document not loaded');
        if (pageNumber < 1 || pageNumber > this.pageCount) throw new Error('Page out of bounds');
        ...
        if (!context) throw new Error('Failed to acquire 2d context for PDF render');

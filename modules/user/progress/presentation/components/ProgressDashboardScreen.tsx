...
export default function ProgressDashboardScreen({ progress, dailyStats = [] }: ProgressDashboardScreenProps) {
    if (!progress) {
        return (
            <div className="flex h-64 items-center justify-center rounded-2xl border border-dashed border-slate-700">
                <span className="text-sm font-medium text-slate-400">No progress data available.</span>
            </div>
        );
    }
        ...
        <div className="space-y-8 animate-in fade-in duration-300">
                    ...
                    <div className="space-y-5">
                        ...
                        <div>
                            <div className="flex justify-between text-sm font-bold text-slate-50">
                                ...
                                <span>{goals.dailyMinutesProgress} / {goals.dailyMinutesTarget} min</span>
                            </div>
                            <div className="w-full bg-[var(--surface-raised)] rounded-full h-2 mt-2 overflow-hidden border border-[var(--border-subtle)]">
                                <div 
                                    ...
                                    style={{ width: `${Math.min(100, (goals.dailyMinutesProgress / Math.max(1, goals.dailyMinutesTarget)) * 100)}%` }}
                                />
                            </div>
                        </div>
                        ...
                        <div>
                            <div className="flex justify-between text-sm font-bold text-slate-50">
                                ...
                                <span>{goals.yearlyBooksProgress} / {goals.yearlyBooksTarget} Books</span>
                            </div>
                            <div className="w-full bg-[var(--surface-raised)] rounded-full h-2 mt-2 overflow-hidden border border-[var(--border-subtle)]">
                                <div 
                                    className="bg-pink-500 h-full rounded-full transition-all duration-300"
                                    style={{ width: `${Math.min(100, (goals.yearlyBooksProgress / Math.max(1, goals.yearlyBooksTarget)) * 100)}%` }}
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-6 rounded-2xl bg-[var(--surface-default)] border border-[var(--border-default)] shadow-sm space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-xl bg-[var(--surface-raised)] border border-[var(--border-subtle)] text-center">
                            ...
                            <div className="text-xl font-bold text-slate-50">{streak.currentDays} Days</div>
                        ...
                        <div className="p-4 rounded-xl bg-[var(--surface-raised)] border border-[var(--border-subtle)] text-center">
                            ...
                            <div className="text-xl font-bold text-slate-50">Level {level.number}</div>
                            <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">{level.totalXp} XP • {level.title}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="p-6 rounded-2xl bg-[var(--surface-default)] border border-[var(--border-default)] shadow-sm space-y-4">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-base font-bold text-slate-50">Recent Achievements</h3>
                </div>

                {recentAchievements.length === 0 ? (
                    <div className="text-sm text-slate-400 text-center py-4">No achievements earned yet. Read more to unlock!</div>
                ) : (
                    <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4">
                        {recentAchievements.map((ach: any) => (
                            <div key={ach.id} className="p-4 rounded-xl border border-indigo-500/35 bg-indigo-600/10 text-center transition-all">
                                <span className="text-3xl block mb-2">🏅</span>
                                <div className="text-xs font-bold text-slate-50 capitalize">{ach.id.replace(/-/g, ' ')}</div>

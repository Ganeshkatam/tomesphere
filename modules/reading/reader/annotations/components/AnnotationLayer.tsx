...
import type { TextContent } from '@/modules/reading/reader/contracts/DocumentEngine';
...
export function AnnotationLayer({ pageNumber, textContent, scale }: AnnotationLayerProps) {
    const { annotations, activeAnnotationId, setActiveAnnotation } = useAnnotationStore();
            ...
            {pageAnnotations.map(annotation => {
                ...
                return (
                    <div key={annotation.id} className="absolute pointer-events-auto">
                        {annotation.anchor.type === 'text' && annotation.anchor.rects.map((rect, idx) => (
                            <div 
                                key={idx}
                                onClick={() => setActiveAnnotation(annotation.id)}
                                className={`absolute cursor-pointer mix-blend-multiply transition-opacity ${
                                    isSelected ? 'opacity-80 ring-2 ring-blue-500' : 'opacity-40 hover:opacity-60'
                                }`}
                                style={{
                                    left: `${rect.x * 100}%`,
                                    top: `${rect.y * 100}%`,
                        ...
                        {annotation.anchor.type === 'area' && (
                            <div 
                                ...
                                className={`absolute cursor-pointer border-2 transition-all ${
                                    isSelected ? 'border-blue-500 bg-blue-500/10' : `border-${baseColor}-500 bg-${baseColor}-500/10 hover:bg-${baseColor}-500/20`
                                }`}
                                style={{

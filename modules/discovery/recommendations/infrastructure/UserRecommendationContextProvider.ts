...
export class UserRecommendationContextProvider implements RecommendationContextProvider {
  ...
  async getForUser(userId: string): Promise<RecommendationContext> {
    const interactions =
      await this.signalRepository.getUserInteractions(userId);

    const completedBookIds = interactions
      .filter((i) => i.completionPercent >= 100)
      .map((i) => i.bookId);
    const currentlyReadingBookIds = interactions
      .filter((i) => i.completionPercent > 0 && i.completionPercent < 100)
      .map((i) => i.bookId);

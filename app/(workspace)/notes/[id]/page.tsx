...
import { createNote, getNoteDetails, updateNote, deleteNote } from '@/modules/planner/academic/actions/academic';
...
export default function NoteEditorPage() {
    ...
    const fetchNote = async () => {
        try {
            const res = await getNoteDetails(noteId);
    ...
    const handleSave = async () => {
        ...
        try {
            if (isNewNote) {
                // Create new note
                const res = await createNote(null, title.trim(), content, tags);
                if (!res.success) throw new Error(res.error);
                showSuccess('Note created!');
            } else {
                // Update existing note
                const res = await updateNote(noteId, title.trim(), content, tags);
                if (!res.success) throw new Error(res.error);

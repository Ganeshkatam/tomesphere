...
module.exports = {
  forbidden: [
    // 1. LAYER RULES
    {
      ...
      comment: 'Domain logic must remain pure and cannot depend on application orchestration.',
      severity: 'error',
    ...
    },
    {
      ...
      comment: 'Domain logic must remain pure and cannot depend on infrastructure.',
      severity: 'error',
      from: { path: '^modules/([^/]+)/domain' },
    ...
    },
    {
      ...
      comment: 'Domain logic must remain pure and cannot depend on presentation.',
      severity: 'error',
      from: { path: '^modules/([^/]+)/domain' },
    ...
    },
    {
      ...
      comment: 'Application layer orchestration should not depend directly on concrete infrastructure implementations (except maybe types/interfaces from shared, but we prefer interfaces in Domain).',
      severity: 'error',
    ...
    // and are allowed to wire domains together.
    {
      ...
      comment: 'Domain layers must be completely isolated — only shared kernel and self-imports allowed.',
      severity: 'error',
      from: { path: '^modules/([^/]+)/domain' },
      to: { 
        path: '^modules/([^/]+)/',
    ...
    },
    {
      ...
      comment: 'Application layers must not reach into other domains, except via shared kernel or explicitly allowed upstream domains.',
      severity: 'error',
      from: { path: '^modules/([^/]+)/application' },
      to: { 
        path: '^modules/([^/]+)/', 
    ...
    },
    {
      ...
      comment: 'Application layer must not interact directly with Supabase clients.',
      severity: 'error',
      from: { path: '^modules/([^/]+)/application' },
      to: { path: '@supabase' }
    },
    ...
    // 4. CROSS-PRESENTATION RULES
    {
      ...
      comment: 'Presentation layers across different domains must not import each other to prevent UI coupling.',
      severity: 'error',
      from: { path: '^modules/([^/]+)/presentation' },
      to: { 

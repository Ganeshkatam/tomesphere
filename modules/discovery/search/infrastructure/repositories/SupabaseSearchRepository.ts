...
export class SupabaseSearchRepository implements SearchRepository {
  ...
  async search(
    ...
    if (query.text) {
      ...
      dbQuery = dbQuery.textSearch("fts_tokens", query.text, {
        config: "english",
      });
    }
    ...
    const documents = (data || []).map((row) => ({
      ...
      availabilityStatus: row.availability_status as
        "available" | "coming_soon" | "out_of_print",
  ...
  async index(document: BookSearchDocument): Promise<void> {
    ...
    await this.supabase.rpc("refresh_search_document", {
      target_book_id: document.bookId,
    });
  }

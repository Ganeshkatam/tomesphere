...
import { ReadingState, ProgressPercentage, ReadingStateValue, ReadingTimeline } from '../value-objects';
import { BookAddedToLibrary, BookFinished, ReadingStateChanged } from '../events';
...
export class LibraryBook extends AggregateRoot<LibraryBookProps> {
    ...
    get state(): ReadingState { return this.props.state; }
    get progress(): ProgressPercentage { return this.props.progress; }
    get timeline(): ReadingTimeline { return this.props.timeline; }
    ...
    static add(id: string, userIdStr: string, bookId: string, initialState: ReadingStateValue = 'want_to_read'): LibraryBook {
        ...
        book.addDomainEvent(new BookAddedToLibrary(book.id, userIdStr, bookId, initialState));
        return book;
    }
    ...
    startReading() {
        if (!this.state.canTransitionTo('currently_reading')) {
            throw new InvalidReadingStateTransition(this.state.value, 'currently_reading');
        }
        const previousState = this.state.value;
        this.props.state = ReadingState.reading();
        this.props.timeline = this.timeline.start();
        this.props.updatedAt = new Date();
        this.addDomainEvent(new ReadingStateChanged(this.id, this.userId.value, this.bookId, previousState, 'currently_reading'));
    }
    ...
    updateProgress(newProgress: number) {
        if (!this.state.allowsProgress()) {
            ...
            if (this.state.value === 'want_to_read' && newProgress > 0) {
                ...
                throw new InvalidReadingStateTransition(this.state.value, 'updateProgress');
            }
        }
    ...
    finish() {
        ...
        this.addDomainEvent(new ReadingStateChanged(this.id, this.userId.value, this.bookId, previousState, 'finished'));
        if (this.timeline.finishedAt) {
            this.addDomainEvent(new BookFinished(this.id, this.userId.value, this.bookId, this.timeline.finishedAt));
        }
    }
    ...
    abandon() {
        ...
        this.addDomainEvent(new ReadingStateChanged(this.id, this.userId.value, this.bookId, previousState, 'abandoned'));
    }
    ...
    restoreToWantToRead() {
        ...
        this.addDomainEvent(new ReadingStateChanged(this.id, this.userId.value, this.bookId, previousState, 'want_to_read'));
    }

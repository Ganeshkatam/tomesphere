...
export class BookMapper {
  static toDomain(raw: BookRow): Book {
    return Book.create({
      ...
      title: raw.title,
      authors: (raw as any).book_authors?.map((ba: any) => ba.authors?.name).filter(Boolean) || [],
      coverUrl: raw.cover_url,
      description: raw.description,
      genres: (raw as any).book_genres?.map((bg: any) => bg.genres?.name).filter(Boolean) || [],
      publishedDate: raw.release_date,
      pageCount: raw.pages,
      isTextbook: raw.is_textbook || false,
      subjects: (raw as any).book_subjects?.map((bs: any) => bs.subjects?.name).filter(Boolean) || [],
      pdfUrl: raw.pdf_url,
      epubUrl: raw.epub_url,

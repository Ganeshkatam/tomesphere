...
describe("ReaderSession Aggregate", () => {
  ...
  it("✓ cannot finish twice", () => {
    ...
    session.complete(createPos("loc-10", 10), 10);
    expect(() => session.complete(createPos("loc-10", 10), 10)).toThrow(
      "already finished",
    );
  });
  ...
  it("✓ cannot update position after finish", () => {
    ...
    session.complete(createPos("loc-10", 10), 10);
    expect(() => session.updatePosition(createPos("loc-15", 15))).toThrow(
      "finished session",
    );
  });
  ...
  it("✓ duplicate events never emitted", () => {
    ...
    const finishEvents = events.filter(
      (e) => e.type === "ReadingSessionCompleted",
    );
  ...
  it("✓ position monotonic", () => {
    ...
    expect(() => session.updatePosition(createPos("loc-5", 5))).toThrow(
      "monotonic",
    );
  });

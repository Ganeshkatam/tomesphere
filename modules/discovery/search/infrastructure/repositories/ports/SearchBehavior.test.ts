...
describe("Search Behavior & Semantics", () => {
  ...
  it("rejects empty query", () => {
    expect(() => SearchQuery.create({ text: "" })).toThrow(
      "Search query must have either text or filters",
    );
  });
  ...
  it("pagination is deterministic", async () => {
    for (let i = 0; i < 15; i++) {
      await repository.index(
        createDoc(`id-${i.toString().padStart(2, "0")}`, "Test Book", "Desc"),
      );
    }

    const q1 = await handler.execute({
      request: { text: "Test", pagination: { limit: 5, offset: 0 } },
    });
    const q2 = await handler.execute({
      request: { text: "Test", pagination: { limit: 5, offset: 5 } },
    });

    expect(q1).toBeDefined();
    expect(q2).toBeDefined();
    if (q1 && q2) {
      expect(q1.results).toHaveLength(5);
      expect(q2.results).toHaveLength(5);
      // Ensure no overlap
      const ids1 = new Set(q1.results.map((r) => r.bookId));
      const overlap = q2.results.some((r) => ids1.has(r.bookId));
      expect(overlap).toBe(false);
      // Ensure stable ordering (00-04, then 05-09)
      expect(q1.results[0].bookId).toBe("id-00");
      expect(q2.results[0].bookId).toBe("id-05");
    }
  });

  it("ranking honors title matches over description matches", async () => {
    await repository.index(
      createDoc("1", "The Magic Ring", "A book about something else."),
    );
    await repository.index(
      createDoc("2", "Some Other Book", "A story containing a magic ring."),
    );
    ...
    if (result) {
      ...
      expect(result.results[0].bookId).toBe("1");
      expect(result.results[1].bookId).toBe("2");
    }
  });

  it("filters combine correctly (author + category + language)", async () => {
    await repository.index(
      createDoc("1", "Book 1", "Desc", ["Alice"], ["Sci-Fi"], "en"),
    );
    await repository.index(
      createDoc("2", "Book 2", "Desc", ["Bob"], ["Sci-Fi"], "en"),
    );
    await repository.index(
      createDoc("3", "Book 3", "Desc", ["Alice"], ["Fantasy"], "en"),
    );
    await repository.index(
      createDoc("4", "Book 4", "Desc", ["Alice"], ["Sci-Fi"], "fr"),
    );

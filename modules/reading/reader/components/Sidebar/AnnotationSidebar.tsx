    ...
    return (
        <div className="w-80 bg-slate-900 border-l border-white/10 flex flex-col h-full animate-in slide-in-from-right">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                {sidebarTab === 'annotations' && annotations.map(({ highlight, note }) => (
                    <div 
                        key={highlight.id}
                        className="bg-slate-800/50 rounded-lg p-3 hover:bg-slate-800 transition-colors cursor-pointer group"
                        onClick={() => service.goToLocation(highlight.selectionAnchor.start)}
                    >
                        {/* Header: Color & Date */}
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: highlight.color }} />
                                {note && <MessageSquare size={12} className="text-indigo-400" />}
                            </div>
                            <div className="flex items-center gap-1 text-xs text-slate-500">
                                <Clock size={12} />
                                {note ? formatDistanceToNow(new Date(note.updatedAt), { addSuffix: true }) : 'Just now'}
                            </div>
                        </div>

                        {/* Highlight Text Preview */}
                        <p className="text-slate-300 text-sm line-clamp-3 mb-2 italic border-l-2 pl-2" style={{ borderColor: highlight.color }}>
                ...
                {sidebarTab === 'bookmarks' && bookmarkViews.map(({ bookmark, isCurrent, preview }) => {
                    // Fallback to auto-generated label
                    const label = bookmark.label || `Bookmark • ${new Date(bookmark.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}`;
                        ...
                        >
                            <div className="flex-1 min-w-0 pr-2">
                                <div className="flex items-center gap-2 mb-1">
                                    <Bookmark size={14} className={isCurrent ? 'text-indigo-400 fill-current' : 'text-slate-400'} />
